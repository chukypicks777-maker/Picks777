...
test('AI destinations reject SSRF, credentials, queries and type confusion', () => {
  ...
  assert.equal(validateAiConfig({ provider: 'openrouter' }).provider, 'custom');
  assert.equal(validateAiConfig({ provider: 'openrouter' }).baseUrl, 'https://vyceai.com/v1');
  assert.equal(validateAiConfig({ provider: 'openrouter', baseUrl: 'https://openrouter.ai/api/v1' }).baseUrl, 'https://vyceai.com/v1');
  assert.equal(validateAiConfig({ provider: 'custom', selectedModel: 'openrouter/free' }).selectedModel, 'deepseek-v4.1');
